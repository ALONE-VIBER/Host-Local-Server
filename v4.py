...
# Function to get the device's hostname
def get_device_hostname(ip):
    try:
        # Try to resolve the host name of the IP address
        hostname = socket.gethostbyaddr(ip)[0]
    except socket.herror:
        # If we can't resolve it, return the IP address as a fallback
        hostname = ip
    return hostname
...
def select_mode(mode):
    session['mode'] = mode
    if mode == 'multiplayer':
        session['room_code'] = str(random.randint(1000, 9999))  # Generate random room code
        return redirect(url_for('waiting_for_opponent'))
...
def game():
    ...
    game_state = session.get('game_state', initialize_game())  # Get game state or initialize it
    device_hostname = get_device_hostname(request.remote_addr)  # Get the hostname of the accessing device
...
def move(cell):
    ...
    session['game_state'] = game_state  # Save updated game state
    return game()
...
def restart():
    ...
    return redirect(url_for('index'))  # Go back to the mode selection page after restart
...
# HTML template for mode selection (Single Player or Multiplayer)
mode_selection_html = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<a href="/mode/single_player"><button>Single Player</button></a><br><br>
<a href="/mode/multiplayer"><button>Multiplayer</button></a><br><br>

</body>
</html>
"""

# HTML template for waiting for the opponent in multiplayer
waiting_html = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<p>Nickname: {{ nickname }} (Room: {{ room_code }})</p>
